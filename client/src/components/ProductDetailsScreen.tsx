      ...
      <div className="pt-8 pb-20 md:pb-8 min-h-screen px-4 md:px-[48px]">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-[24px]">
            ...
            {/* Product Info - Preserved layout structure but simplified details hierarchy */}
            <div className="lg:col-span-5 flex flex-col space-y-8">
              ...
              {/* COLLAPSIBLE SPECIFICATIONS ACCORDION (Specs collapse cleanly) */}
              <div className="border-t border-[#c7c7bf] pt-6 text-left">
                <button 
                  onClick={() => setIsSpecsOpen(!isSpecsOpen)}
                  className="w-full flex items-center justify-between text-left font-bold text-[14px] leading-[1.4] tracking-[0.04em] text-[#020302] uppercase font-sans cursor-pointer"
                >
                ...
                {isSpecsOpen && (
                  <div className="mt-4 space-y-6">
                    ...
                    {/* Color Swatches - Rendered cleanly inside specifications accordion */}
          ...
          {/* MANAGE VARIANTS SECTION - Positioned here to dominate screen */}
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="text-left">
                          ...
                          <p className="font-bold text-[12px] leading-[1.4] text-[#ba1a1a] font-sans mt-1">Stock: 0 (OUT OF STOCK)</p>
                        </div>
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-2 gap-8 text-left">
                        <div>
                          <p className="font-bold text-[12px] leading-[1.4] text-[#5e5e5d] uppercase mb-1 font-sans tracking-wide">Buy</p>
                          <p className="font-bold text-[14px] leading-[1.4] tracking-[0.04em] text-[#020302] font-sans">{formatValK(v.buyingPrice)}</p>
                        </div>
                        <div>
                          <p className="font-bold text-[12px] leading-[1.4] text-[#5e5e5d] uppercase mb-1 font-sans tracking-wide">Sell</p>
                          <p className="font-bold text-[14px] leading-[1.4] tracking-[0.04em] text-[#020302] font-sans">{formatValK(v.sellingPrice)}</p>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-[12px] leading-[1.4] font-normal text-[#5e5e5d] font-sans mt-1">
                          Stock: <span className="font-bold text-[#1b1c1c]">{v.quantity}</span>
                        </p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-2 gap-8 text-left">
          ...
          {/* STOCK TRANSACTIONS ACTIVITY FEED (TIMELINE - Preserves exact border/colors visual styles) */}
          <section className="mt-16 mb-12">
            <div className="border-b border-[#c7c7bf] pb-4 mb-8 text-left">
            ...
            {product.history && product.history.length > 0 ? (
              <div className="relative border-l-2 border-[#c7c7bf] ml-4 pl-6 space-y-6 text-left">
                {product.history.map((log: any, idx: number) => {
                  const parts = log.text.split(' — ');
                  const actionDesc = parts[0] || 'Stock updated';
                  ...
                  return (
                    <div key={idx} className="relative">
                      ...
                      <span className={`absolute -left-[32px] top-1 flex h-4.5 w-4.5 items-center justify-center rounded-full border bg-white
                        ${isSell ? 'border-[#ba1a1a]' : 'border-emerald-600'}`}
                      >
                        <span className={`h-2 w-2 rounded-full ${isSell ? 'bg-[#ba1a1a]' : 'bg-emerald-600'}`} />
                      </span>

                      {/* Timeline Content Detail Box */}
                      <div className="bg-white border border-[#c7c7bf] p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        <div>
                          <h4 className="font-bold text-[14px] leading-[1.4] tracking-[0.04em] text-[#020302] font-sans">
                            {actionDesc}
                          </h4>
                          <p className="text-[12px] text-[#5e5e5d] font-sans mt-0.5 uppercase tracking-wide">
                            Configuration: {variantDetails}
                          </p>
                        </div>
                        <div className="text-left sm:text-right">
                          <span className="text-[12px] font-bold text-[#5e5e5d] font-sans bg-neutral-100 px-2 py-0.5 rounded-sm">
                            {dateStr}
                          </span>
                        </div>
                      </div>
              ...
              <div className="text-center py-8 text-[#5e5e5d] font-sans border border-dashed border-[#c7c7bf]">
